...
const EditUserForm = ({ id, data }) => {
  ...
  return (
    <form onSubmit={saveData} className='edit-user-form'>
      ...
      <input type='text' placeholder={data.nombre} onChange={(e) => { setUserName(e.target.value) }} />
      <input type='text' placeholder={data.biografia} onChange={(e) => { setUserBio(e.target.value) }} />
